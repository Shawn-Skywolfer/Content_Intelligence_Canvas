...
function CanvasView({ project, canvas, selectedIds, setSelectedIds, onChange, onRefresh, setStatus }: {
...
function SettingsView({ sources, setSources, setStatus }: { sources: Source[]; setSources: (sources: Source[]) => void; setStatus: (value: string) => void }) {
  const [providers, setProviders] = useState<Provider[]>([]); const [selectedProvider, setSelectedProvider] = useState(""); const [protect, setProtect] = useState(true); const [sourceName, setSourceName] = useState("本地知识库"); const [sourcePath, setSourcePath] = useState(""); const [busy, setBusy] = useState(false);
  const [form, setForm] = useState({ name: "自定义模型", protocol: "openai_compatible", base_url: "", api_key: "", model_name: "", enabled: true, is_external: true, temperature: .3, max_tokens: 3000, timeout_seconds: 60 });
  ...
  function setFormFromProvider(provider: Provider) { setForm({ name: provider.name, protocol: provider.protocol, base_url: provider.base_url, api_key: "", model_name: provider.model_name, enabled: provider.enabled, is_external: provider.is_external, temperature: provider.temperature, max_tokens: provider.max_tokens, timeout_seconds: provider.timeout_seconds }); }
  ...
  return <><PageHeader eyebrow="设置" title="知识源、大模型与数据边界" description="支持任意 OpenAI 兼容模型服务；未配置模型时，核心流程仍可使用本地模板运行。" />
    <div className="settings-grid"><section className="card settings-card"><div className="section-title"><h2>本地知识源</h2><span>只读，不修改源文件</span></div><form onSubmit={addSource}><label>知识源名称<input value={sourceName} onChange={(e) => setSourceName(e.target.value)} /></label><label>本地知识库文件夹路径<input value={sourcePath} onChange={(e) => setSourcePath(e.target.value)} placeholder="例如 D:\\Knowledge\\wiki" /></label><button className="primary" disabled={busy || !sourcePath.trim()}>添加知识源</button></form><div className="source-list">{sources.map((source) => <div key={source.id}><div><strong>{source.name}</strong><small>{source.root_path}</small></div><button className="secondary" disabled={busy} onClick={() => refreshSource(source.id)}>刷新索引</button></div>)}</div></section>
      <section className="card settings-card"><div className="section-title"><h2>自定义大模型</h2><button className="text-button" onClick={() => { setSelectedProvider(""); setForm({ name: "自定义模型", protocol: "openai_compatible", base_url: "", api_key: "", model_name: "", enabled: true, is_external: true, temperature: .3, max_tokens: 3000, timeout_seconds: 60 }); }}>新建配置</button></div>{providers.length > 0 && <label>已保存配置<select value={selectedProvider} onChange={(e) => { setSelectedProvider(e.target.value); const provider = providers.find((item) => item.id === e.target.value); if (provider) setFormFromProvider(provider); }}><option value="">新建配置</option>{providers.map((provider) => <option key={provider.id} value={provider.id}>{provider.name} · {provider.model_name}</option>)}</select></label>}
        <form onSubmit={saveProvider} className="provider-form"><label>配置名称<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label>接口协议<select value={form.protocol} onChange={(e) => setForm({ ...form, protocol: e.target.value })}><option value="openai_compatible">OpenAI 兼容接口</option><option value="custom">自定义兼容接口</option></select></label><label>接口基础地址<input value={form.base_url} onChange={(e) => setForm({ ...form, base_url: e.target.value })} placeholder="https://api.example.com/v1" /></label><label>模型名称<input value={form.model_name} onChange={(e) => setForm({ ...form, model_name: e.target.value })} placeholder="模型标识" /></label><label>接口密钥<input type="password" value={form.api_key} onChange={(e) => setForm({ ...form, api_key: e.target.value })} placeholder={selectedProvider ? "留空则保留现有密钥" : "可选：本地服务可不填"} /></label><div className="config-grid"><label>温度<input type="number" min="0" max="2" step="0.1" value={form.temperature} onChange={(e) => setForm({ ...form, temperature: Number(e.target.value) })} /></label><label>最大输出<input type="number" min="128" value={form.max_tokens} onChange={(e) => setForm({ ...form, max_tokens: Number(e.target.value) })} /></label><label>超时秒数<input type="number" min="5" max="300" value={form.timeout_seconds} onChange={(e) => setForm({ ...form, timeout_seconds: Number(e.target.value) })} /></label></div><label className="check"><input type="checkbox" checked={form.is_external} onChange={(e) => setForm({ ...form, is_external: e.target.checked })} />这是外部模型服务</label><div className="form-actions"><button className="primary" disabled={busy || !form.base_url || !form.model_name}>保存配置</button><button type="button" className="secondary" disabled={busy || !selectedProvider} onClick={testProvider}>测试连接</button></div></form></section>
